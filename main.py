...
# admin routes stay open (dev mode) — do not leave unset in prod.
# ═══════════════════════════════════════════════════════
...
class _SmartSarwanSwarm:
    ...
    def _register_all(self):
        for claw_key, claw_info in self.CLAWS.items():
            for i in range(1, claw_info["agents"] + 1):
                ...
                linked = [mod for mod, agents in self.MODULE_AGENT_MAP.items() if agent_id in agents]
                required = [key for key, agents in self.KEY_AGENT_MAP.items() if agent_id in agents]
    ...
    def get_status(self):
        ...
        memory_saved = round(unloaded * 0.5, 1)
        return {
...
# ═══════════════════════════════════════════════════════
# 🦁 CORS — restricted to known frontends, not wide open
# Add more origins to this list as you add frontends.
# Set EXTRA_CORS_ORIGINS in Railway (comma-separated) for
# temporary/dev origins without editing code.
# ═══════════════════════════════════════════════════════
ALLOWED_ORIGINS = [
...
# ═══════════════════════════════════════════════════════
# 🦁 ADMIN MODULE — protected by ADMIN_API_KEY
# Send header: X-Admin-Key: <your ADMIN_API_KEY>
# ═══════════════════════════════════════════════════════
@app.get("/api/admin/")
async def admin_root(request: Request):
    if not _check_admin_auth(request):
        return JSONResponse({"error": "Unauthorized — X-Admin-Key header required"}, status_code=401)
...
async def mp_status(app_id: str):
    app_data = MiniAuth.APPROVED_APPS.get(app_id)
    return {"app_id": app_id, "status": app_data["status"]} if app_data else {"error": "App not found"}
...
async def mp_admin_apps(request: Request, status: str = None):
    if not _check_admin_auth(request):
        return JSONResponse({"error": "Unauthorized — X-Admin-Key header required"}, status_code=401)
    return {"apps": DeveloperPortal.get_all_apps(status)}

@app.post("/api/miniprogram/admin/approve/{app_id}")
async def mp_admin_approve(app_id: str, request: Request):
    if not _check_admin_auth(request):
        return JSONResponse({"error": "Unauthorized — X-Admin-Key header required"}, status_code=401)
