    ...
    try:
        ...
        if "callback_query" in data:
            ...
            if query_data == "status":
                ...
                _telegram_send_message(chat_id, NL.join(text_lines))

            elif query_data == "weather":
                _telegram_send_message(chat_id, "Weather: City batao! Example: /weather Delhi")
                ...
                if NEWSDATA_API_KEY:
                    try:
                        ...
                        text_lines = ["Latest News", ""]
                        for i, article in enumerate(articles, 1):
                            text_lines.append(f"{i}. {article.get('title', 'No title')}")
                            text_lines.append(f"   {article.get('description', 'No description')[:100]}...")
                            text_lines.append("")
                        _telegram_send_message(chat_id, NL.join(text_lines))
                    except Exception as e:
                ...
                _telegram_send_message(chat_id, "Mandi Bhav: State batao! Example: /mandi Punjab")

            elif query_data == "ai_chat":
                _telegram_send_message(chat_id, "AI Chat: Kuch bhi poochho! Main jawab dunga.")

            elif query_data == "voice":
                _telegram_send_message(chat_id, "Voice: Voice message bhejo! Main transcribe karke AI response dunga.")

            elif query_data == "tax":
                _telegram_send_message(chat_id, "Tax Calculator: Income batao! Example: /tax 500000")

            elif query_data == "plant":
                _telegram_send_message(chat_id, "Plant ID: Plant ki photo bhejo! Main identify karunga.")
        ...
        if text == "/start":
            welcome_lines = [
                ...
                "Niche buttons se kuch bhi try karo!"
            ]
            _telegram_send_message(chat_id, NL.join(welcome_lines), MAIN_KEYBOARD)
            return {"status": "ok"}
            ...
            help_lines = [
                ...
                "/news - Latest news",
                "/mandi <state> - Mandi bhav",
                "/tax <income> - Tax calculate",
                "/status - System status",
                "/ai <question> - AI chat",
                "Voice message bhejo - STT + AI"
            ]
            _telegram_send_message(chat_id, NL.join(help_lines))
            return {"status": "ok"}
            ...
            status_lines = [
                ...
                f"Active: {status['active_running']}",
                f"Idle: {status['idle']}",
                f"Active APIs: {sum(1 for v in AVAILABLE_KEYS.values() if v)}/{len(AVAILABLE_KEYS)}",
                f"Time: {datetime.now().strftime('%H:%M:%S')}"
            ]
            _telegram_send_message(chat_id, NL.join(status_lines))
            return {"status": "ok"}
            ...
            if OPENWEATHER_API_KEY:
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        _telegram_send_message(chat_id, NL.join(weather_lines))
                    else:
            ...
            if NEWSDATA_API_KEY:
                try:
                    ...
                    news_data = resp.json()
                    articles = news_data.get("results", [])[:5]
                    news_lines = ["Latest News", ""]
                    for i, article in enumerate(articles, 1):
                        news_lines.append(f"{i}. {article.get('title', 'No title')}")
                        news_lines.append(f"   {article.get('description', 'No description')[:80]}...")
                        news_lines.append("")
                    _telegram_send_message(chat_id, NL.join(news_lines))
                except Exception as e:
                    _telegram_send_message(chat_id, f"News error: {str(e)[:100]}")
            else:
                _telegram_send_message(chat_id, "News API key missing")
            ...
            if MANDI_API_KEY:
                try:
                    ...
                    mandi_lines = [f"Mandi Bhav - {state}", ""]
                    for i, record in enumerate(records[:5], 1):
                        mandi_lines.append(f"{i}. {record.get('commodity', 'Unknown')}")
                        mandi_lines.append(f"   Rs.{record.get('modal_price', 'N/A')}/quintal")
                        mandi_lines.append(f"   {record.get('district', 'N/A')}, {record.get('market', 'N/A')}")
                        mandi_lines.append("")
                    _telegram_send_message(chat_id, NL.join(mandi_lines))
                except Exception as e:
                    _telegram_send_message(chat_id, f"Error: {str(e)[:100]}")
            else:
            ...
            try:
                ...
                tax_lines = [
                    ...
                    f"Tax: Rs.{tax:,.2f}",
                    f"Health Cess (4%): Rs.{cess:,.2f}",
                    f"Total Tax: Rs.{total:,.2f}",
                    f"Take Home: Rs.{income - total:,.2f}"
                ]
                _telegram_send_message(chat_id, NL.join(tax_lines))
            except:
            ...
            if GROQ_API_KEY:
                try:
                    ...
                    ai_lines = ["AI Response:", "", ai_response]
                    _telegram_send_message(chat_id, NL.join(ai_lines))
            ...
            for uid in USER_PREFERENCES.keys():
                _telegram_send_message(uid, f"Broadcast{NL}{NL}{broadcast_text}")
                sent += 1
        ...
        if now.hour == 4 and now.minute == 0:
            ...
            try:
                if NEWSDATA_API_KEY:
                    ...
                    news_data = resp.json()
                    articles = news_data.get("results", [])[:5]
                    news_lines = ["4 AM Daily News", "", "Singh Ji AI"]
                    for i, article in enumerate(articles, 1):
                        news_lines.append("")
                        news_lines.append(f"{i}. {article.get('title', 'No title')}")
                        news_lines.append(f"   {article.get('description', 'No description')[:100]}...")
                    for uid in USER_PREFERENCES.keys():
                        _telegram_send_message(uid, NL.join(news_lines))
...
async def admin_broadcast(request: Request):
    ...
    sent = 0
    for uid in USER_PREFERENCES.keys():
        _telegram_send_message(uid, f"Admin Broadcast{NL}{NL}{message}")
        sent += 1
