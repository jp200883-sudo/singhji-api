...
def load_module(name, modules_dir):
    ...
    try:
        if p.is_dir():
            ...
            info["status"] = "not_found"; info["error"] = f"Not found: {p}"; return info
...
if modules_dir and modules_dir.exists():
    ...
    for item in sorted(modules_dir.iterdir()):
        ...
        if item.is_file() and item.suffix not in ('.py', ''): continue
...
# ═══════════════════════════════════════════════════════════════════
# 🎯 SPECIFIC ROUTES FIRST — CATCH-ALL LAST!
# ═══════════════════════════════════════════════════════════════════

@app.api_route("/api/health", methods=["GET", "HEAD"])
def health_check():
    return {"status": "🦁 LIVE", "version": "7.0.0", "loaded": len(loaded), "total": len(MODULES), "modules": loaded}

@app.api_route("/api/status", methods=["GET", "HEAD"])
def status():
    return {"loaded_count": len(loaded), "failed_count": len(failed), "loaded": loaded, "failed": failed, "details": {k: {"status": v["status"], "type": v.get("type"), "error": v.get("error", "")[:200]} for k, v in MODULES.items()}}

@app.api_route("/", methods=["GET", "HEAD"])
def root():
    return {"name": "Singh Ji AI Ultra v7.0", "loaded": len(loaded), "total": len(MODULES), "status": "🦁 LIVE", "endpoints": {"health": "/api/health", "status": "/api/status", "module_api": "/api/{module_name}"}}

# CATCH-ALL ROUTE — MUST BE LAST!
@app.api_route("/api/{m}", methods=["GET", "POST", "HEAD"])
async def api(m: str, request: Request):
    if m not in MODULES or MODULES[m]["status"] != "loaded":
        return JSONResponse(status_code=404, content={"error": f"Module '{m}' not found or not loaded"})
