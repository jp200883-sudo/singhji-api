...
class Config:
    ...
    WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "singhji_webhook_secret_2024")
    WEBHOOK_URL = os.getenv("WEBHOOK_URL", "")
    API_BASE_URL = os.getenv("API_BASE_URL", "https://your-api-url.com")
...
class ConversationManager:
    ...
    def get_state(self, user_id):
        return self.states.get(user_id, {"state": ConversationState.IDLE, "data": {}, "timestamp": None})
    ...
    def cleanup(self):
        now = time.time()
        expired = [uid for uid, data in self.states.items() if now - data.get("timestamp", 0) > 300]
        for uid in expired:
...
class LanguageManager:
    ...
    def get_lang_name(self, user_id):
        return self.lang_names.get(self.get_lang(user_id), "Hindi")

    def get_system_prompt(self, user_id):
        lang = self.get_lang(user_id)
        prompts = {
            "hi": "You are Singh Ji AI - India's most powerful AI assistant! Respond in Hinglish (Hindi + English mix). Be friendly, helpful, and respectful. Use simple language that rural Indians can understand.",
            "en": "You are Singh Ji AI - India's most powerful AI assistant! Respond in English. Be friendly, helpful, and respectful. Give clear, practical answers.",
            "ta": "You are Singh Ji AI - India's most powerful AI assistant! Respond in Tamil language. Be friendly, helpful, and respectful. Use simple Tamil that everyone can understand.",
            "te": "You are Singh Ji AI - India's most powerful AI assistant! Respond in Telugu language. Be friendly, helpful, and respectful. Use simple Telugu that everyone can understand.",
            "mr": "You are Singh Ji AI - India's most powerful AI assistant! Respond in Marathi language. Be friendly, helpful, and respectful. Use simple Marathi that everyone can understand.",
            "bn": "You are Singh Ji AI - India's most powerful AI assistant! Respond in Bengali language. Be friendly, helpful, and respectful. Use simple Bengali that everyone can understand.",
            "gu": "You are Singh Ji AI - India's most powerful AI assistant! Respond in Gujarati language. Be friendly, helpful, and respectful. Use simple Gujarati that everyone can understand.",
            "pa": "You are Singh Ji AI - India's most powerful AI assistant! Respond in Punjabi language. Be friendly, helpful, and respectful. Use simple Punjabi that everyone can understand."
        }
...
class UserMemory:
    ...
    def set(self, user_id, content, metadata=None):
        self._cleanup()
...
class BotAnalytics:
    ...
    def track_message(self, user_id, message_type):
    ...
    def track_module_usage(self, module_name):
...
class AIBrain:
    ...
    async def get_response(self, user_text, user_id, user_name="User", chat_type="private"):
...
class TTSEngine:
    ...
    async def text_to_speech(self, text, user_id=None):
        try:
...
# API HELPERS
async def fetch_and_send_weather(update, city):
    try:
...
async def fetch_and_send_mandi(update, state):
    try:
        response = requests.get(
...
async def calculate_and_send_tax(update, income):
    try:
        response = requests.get(
...
async def fetch_and_send_gold(update, city="India"):
    try:
        response = requests.get(
...
async def fetch_and_send_fuel(update, city):
    try:
        response = requests.get(
...
@error_handler_decorator
@rate_limit_check
async def use_module_command(update, context):
    args = context.args
    user = update.effective_user
    analytics.track_message(user.id, "command")
    if not args:
...
@error_handler_decorator
async def recall_command(update, context):
    user = update.effective_user
    ...
    try:
        ...
        if voice_reply:
            await update.message.reply_voice(
                ...
                caption=f"{transcript[:50]}... -> {ai_response['text'][:50]}..."
            )
        else:
            await update.message.reply_text(
        ...
        await processing_msg.edit_text(
            "Voice process nahi ho paya!\n\nText se try karo!"
        )

# TEXT CHAT HANDLER
@error_handler_decorator
@rate_limit_check
async def text_chat_handler(update, context):
    user = update.effective_user
...
Examples:
- Parents hug
- Doctor check (with parents present)
- Friend high-five
...
Examples:
- Touching under underwear/vest
- Forced hugging
- Trying to touch alone
- Showing vulgar videos
...
1. Shout - Say 'Help' loudly
2. Run - Towards safe place
3. Tell - Parents, Teacher, Police
4. Call 1098
5. Never fear - It's not your fault!
...
3. NCPCR e-Box: ncpcr.gov.in
4. Action under POCSO Act
...
Your voice will be heard!"""
            ...
            bachpan_text = """BACHPAN - Child Safety

Government Helplines (100% REAL):

1098 - Childline (24x7, FREE)
100 - Police (24x7, FREE)
181 - Women Helpline
108 - Ambulance
1930 - Cyber Crime
1800-572-1929 - NCPCR
...
# BACHPAN COMMAND
@error_handler_decorator
async def bachpan_command(update, context):
    user = update.effective_user
    analytics.track_message(user.id, "command")
    bachpan_text = """BACHPAN - Child Safety
...
async def payment_status(order_id):
    try:
        return {
            "order_id": order_id,
            "status": "pending",
