...
# Auto-promo messages
PROMO_MESSAGES = [
    "🦁 *Singh Ji AI Ultra v8.0* — 95 Modules Active!\n\n💬 AI Chat | 🎙️ Voice | 📰 News | 🌤️ Weather\n\n👉 @SinghJiAIBot",
    "🔥 *Desi AI Bot* — Hindi + English dono mein!\n\n🎙️ Voice message bhejo, jawab suno!\n\n👉 @SinghJiAIBot",
    "📰 *Roz 4 baje News* — Auto delivery!\n\n🦁 Singh Ji AI — Apna desi AI assistant\n\n👉 @SinghJiAIBot",
]
...
async def get_ai_response(user_text: str, user_id: int, user_name: str = "User") -> str:
    ...
    try:
            ...
            messages=[
                {
                    ...
                    User: {user_name}. Previous: {context}"""
                },
        ...
        user_memory[user_id] = f"Asked: {user_text[:100]}"
...
ACTIVE_MODULES = {
    "🌤️ Weather": "weather", "📰 News": "news", "🤖 AI Chat": "ai_chat",
    ...
    "🛡️ Guard Agent": "guard_agent", "⚡ Trishul": "trishul", "🛒 Trolley": "trolley",
    "📅 Schedule": "schedule", "📋 Daily Report": "daily_report", "🚨 Emergency": "emergency",
    "🏛️ Govt Schemes": "govt", "📺 Singh Ji TV": "singhji_tv", "🎙️ Voice": "voice",
    ...
    "📊 Claw 7": "claw_7", "🎙️ Voice CMD": "voice_cmd", "🎙️ Voice TTS": "voice_tts",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start command — with user tracking"""
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📢 Share Bot", url=f"https://t.me/share/url?url=https://t.me/{context.bot.username}&text=🦁 Singh Ji AI Ultra - Desi AI Bot!")],
    ]
...
async def use_module(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Module use karo — with timeout handling"""
    args = context.args
    if not args:
        await update.message.reply_text(
    ...
    processing_msg = await update.message.reply_text(f"⏳ `{module}` load ho raha hai...")

    try:
        timeout = 60 if module in ["aavishkar", "image"] else 30
        
        response = requests.get(
            f"{API_BASE_URL}/modules/{module}/",
            params={"q": query, "prompt": query},
            timeout=timeout
        )
        ...
        await processing_msg.edit_text(f"⏱️ *{module.upper()} Timeout*\n\nThodi der baad try karo!")
    except Exception as e:
        await processing_msg.edit_text(f"❌ *Error:* `{str(e)[:500]}`")
...
async def recall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🧠 *Tumhari Memory:*\n\n📝 {memory}\n\n*Naya yaad rakhne ke liye:* `/remember <text>`",
        parse_mode="Markdown"
    )
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🧠 Memory Users: {len(user_memory)}\n"
        f"🤖 Status: Online\n"
        f"🚀 Version: v8.0",
        parse_mode="Markdown"
    )
...
# ═══════════════════════════════════════════════════════
# TEXT CHAT HANDLER — AI JAWAB + VOICE + PROMO
# ═══════════════════════════════════════════════════════
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "list_modules":
        ...
        time_map = {
            "news_4am": "4:00 AM", "news_6am": "6:00 AM",
            "news_8am": "8:00 AM", "news_9pm": "9:00 PM", "news_off": "OFF"
        }
        time_set = time_map.get(query.data, "4:00 AM")
        await query.edit_message_text(f"✅ *News Updated*\n\nAb: *{time_set}* 📰", parse_mode="Markdown")
